...
1. 가설을 생성
    * 데이터 탐색을 통하여 데이터의 특성들을 확인할 수 있었음
    * 이러한 데이터를 바탕으로 이 분야의 도메인 지식(knowledge)를 활용하여 새로운 가설을 생성함
...
1. 가설 1 "가족과의 관계가 생존에 영향을 주었을 것이다."
    * 타이타닉 영화를 보면 여주인공의 약혼자가 살기위해 모르는 딸아이의 아빠인척을 하는 장면이 나온다.
      이처럼 가족의 생존확률이 높으면 자신의 생존 확률이 높을 것이라고 추정해 볼 수 있다.

1. 가설 2 "여자와 아이의 생존 확률이 높았을 것이다."
...
1 - pchisq((765.4-464.5), df=(568-554))  ## 0 유의미한 차이가 있고, 모델이 잘 수행한다고 봐야함
...
앞서 우리는 '여자와 아이들 우선' 가설을 생성함, Title로 가설에 중요한 변수를 적용함
...
'여자와 아이들 우선'의 가설을 검증하기 위하여 I()함수를 이용하여 몇 가지 독립변수 조합을 추가
